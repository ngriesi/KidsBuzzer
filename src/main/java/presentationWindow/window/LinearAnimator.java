...
public class LinearAnimator {
    ...
    LinearAnimator(OpenGlRenderer openGlRenderer) {
    ...
    public Animation colorFade(Color newColor, ChildItem item, int duration) {
        return startAnimation(new ColorAnimation(item.getColor(), newColor, duration, nextValue -> item.setColorScheme(new ColorScheme(nextValue)), Animation.AnimationCurve.LINEAR));
    }

    public Animation fadeOut(ChildItem item, int duration) {
        return startAnimation(new FloatAnimation(item.getOpacity(), 0f, duration, item::setOpacity, Animation.AnimationCurve.LINEAR));
    }

    public Animation fadeIn(ChildItem item, int duration) {
        return startAnimation(new FloatAnimation(item.getOpacity(), 1f, duration, item::setOpacity, Animation.AnimationCurve.LINEAR));
    }

    public Animation scaleSize(Vector2f newSize, ChildItem item, int duration) {
        return startAnimation(new Vector2fAnimation(item.getSize(),newSize,duration,item::setSize, Animation.AnimationCurve.LINEAR));
    }

    public Animation moveTo(Vector2f newPosition, ChildItem item, int duration) {
        return startAnimation(new Vector2fAnimation(item.getPosition(),newPosition,duration,item::setPosition, Animation.AnimationCurve.LINEAR));
    }

    public Animation moveXTo(float newX, ChildItem item, int duration) {
        return startAnimation(new FloatAnimation(item.getXPosition(), newX, duration, item::setXPosition, Animation.AnimationCurve.LINEAR));
    }

    public Animation moveYTo(float newY, ChildItem item, int duration) {
        return startAnimation(new FloatAnimation(item.getYPosition(), newY, duration, item::setYPosition, Animation.AnimationCurve.LINEAR));
    }

    public Animation scaleHeightTo(float newHeight, ChildItem item, int duration) {
        return startAnimation(new FloatAnimation(item.getHeight(),newHeight,duration,item::setHeight, Animation.AnimationCurve.LINEAR));
    }

    public Animation scaleWidthTo(float newWidth, ChildItem item, int duration) {
        return startAnimation(new FloatAnimation(item.getHeight(),newWidth,duration,item::setWidth, Animation.AnimationCurve.LINEAR));
    }
